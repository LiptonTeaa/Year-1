...
function drawMonochromaticPallette(hue, startX, startY, size)
{
  let numberOfColors = 8; // number of colors in this pallette
  let saturation = 80; // how "colorful" or gray this color is, from 0-100
  let maxBrightness = 100; // maximum brightness value
...
function draw() {
  ...
  fill(180); // gray
  textSize(24);
  text("Monochromatic with hue = " + hue, 10, 40-12);

  // draw with hue = 160
  drawMonochromaticPallette(hue, 30, 42, 300);

  hue = 220; // update hue for next pallette

  fill(180); // gray
  textSize(24);
  text("Monochromatic with hue = " + hue, 10, 134-12);
  // draw with hue = 220
  drawMonochromaticPallette(hue, 10, 128, 300);
